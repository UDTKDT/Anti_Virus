//g++ -o UdkdAgent firewall.* main.* error_codes.* ini.h VariadicTable.h 
...
int StartFirewall(){
    ...
    std::vector<std::string> optionList = {"iptables -A ", " -d " ," -p tcp --dport ", " -j "};
    ...
    for (auto& it : ini) {
        ...
        for (auto& it2 : collection) {
            ...
            std::string v =(value == "in") ? "INPUT" : (value == "out") ? "OUTPUT" :(value == "permit") ? "ACCEPT" : (value == "deny") ? "DROP" : value;
...
int ConfigureFirewall(){
    ...
    while (true){
        ...
        if (cmd != command_map.end()){
            ...
            std::cout << "Error : " << GetErrorMessage(ERROR_INVALID_INPUT) << std::endl;
            return ERROR_INVALID_INPUT;
        }
    }
...
int UpdateRule(std::vector<std::string>& words){
    ...
    std::cout << GetErrorMessage(ERROR_INVALID_INPUT) << std::endl;
    return ERROR_INVALID_INPUT;
